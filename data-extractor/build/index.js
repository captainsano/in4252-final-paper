...
reader.on('line', (line) => {
    if (line.startsWith('[')) {
        let [, tag, value] = line.match(/\[([a-zA-Z0-9]+)\s+\"(.*)\"\]/);
        tag = tag.toLowerCase();
        if (value === '?') {
    ...
    if (line.endsWith('1-0') || line.endsWith('0-1') || line.endsWith('1/2-1/2') || line.endsWith('*')) {
        ...
        writeRaw(Object.assign({}, currentGame));
        writeElo(Object.assign({}, currentGame));
        currentGame = {};
    }
});
